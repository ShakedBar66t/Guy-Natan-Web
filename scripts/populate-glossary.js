...
// Check if script is running in browser
if (typeof window !== 'undefined') {
  // Create UI elements for the browser
  const runButton = document.createElement('button');
  runButton.textContent = 'הוסף מונחים למילון';
  runButton.className = 'bg-[#32a191] text-white px-4 py-2 rounded-md hover:bg-[#2a8a7c] transition-colors';
  runButton.onclick = populateGlossary;
  
  const statusDiv = document.createElement('div');
  statusDiv.id = 'status';
  statusDiv.className = 'mt-4 text-[#002F42]';
  
  document.body.appendChild(runButton);
  document.body.appendChild(statusDiv);
  
  console.log('👋 Run the script by clicking the "הוסף מונחים למילון" button');
} else {
  // Run the script directly when in Node.js environment
  populateGlossary();
}
